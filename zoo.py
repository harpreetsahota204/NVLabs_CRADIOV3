...
class TorchRadioModel(fout.TorchImageModel):
    ...
    def _load_model(self, config):
        ...
        checkpoint = torch.load(config.model_path, map_location=self._device)
    ...
    def _predict_all(self, imgs):
        ...
        if isinstance(output, dict):
            return [{
                "summary": batch_summary[i].detach().cpu().numpy(),
                "spatial": batch_spatial[i].detach().cpu().numpy()
...
class RadioOutputProcessor(fout.OutputProcessor):
    ...
    def __call__(self, output, frame_size, confidence_thresh=None):
        ...
        Args:
            output: tensor or dict from RADIO model
        ...
        """
        if isinstance(output, dict):
            # Both summary and spatial features
            batch_size = output["summary"].shape[0]
            return [{
                "summary": output["summary"][i].detach().cpu().numpy(),
                "spatial": output["spatial"][i].detach().cpu().numpy()
